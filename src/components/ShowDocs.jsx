...
export default function ShowDocs ({documents}) {
    ...
    function checkFilter () {
        if (documents.type === "import" && showsnotify === "false") {
    ...
    const DocContents = () => {
        ...
        content.map((item) => ( // -- Add grid hoverboxes here? Show based on hover value?
            ...
            onClick={() => {
                ...
                <div className="char-limit">{checkNotify(item[1].consignee, item[1].consignor, item[1].notifyParty)}</div>
                <div>{item[1].m}</div>
